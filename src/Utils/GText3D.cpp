...
void t3dInit() {
	if (font == NULL) {
		...
		std::string package_path = ros::package::getPath(Antipatrea::Constants::KW_ProjectName_);
        auto FONT_FILE = package_path + "/" + Antipatrea::Constants::FONT_FILE;

		input.open(FONT_FILE.c_str(), istream::binary);
